...
class CloudHttpClient {
  ...
  private val API_KEY = Option(System.getenv("API_KEY")).orElse(throw new RuntimeException("API_KEY variable is required for new deployment")).get
  val logger: Logger = LoggerFactory.getLogger("httpClient")

  def preparePayload(config: Config): String = {
    logger.info(s"preparePayload: Using ${deployPayloadTemplate} payload template")
    ...
    val payload = template
      ...
      .update('resources/ 'elasticsearch/ element(0)/ 'plan/ 'elasticsearch/ 'version ! set[String](config.getString("version")))
      .update(
        'resources/ 'elasticsearch/ element(0)/ 'plan/ 'cluster_topology/ element(0)/ 'size/ 'value
      ...
      .update('resources/ 'kibana/ element(0)/ 'plan/ 'kibana/ 'version ! set[String](config.getString("version")))
      .update(
        'resources/ 'kibana/ element(0)/ 'plan/ 'cluster_topology/ element(0)/ 'size/ 'value
        ! set[Int](config.getInt("kibana.memory"))
      )
      .update('resources/ 'apm/ element(0)/ 'plan/ 'apm/ 'version ! set[String](config.getString("version")))
  ...
  def createDeployment(payload: String): Map[String, String] = {
    ...
    val meta = Map(
      ...
      "username" -> responseString.extract[String]('resources/ element(0)/ 'credentials/ 'username),
      "password" -> responseString.extract[String]('resources/ element(0)/ 'credentials/ 'password)
    )

    logger.info(s"createDeployment: deployment ${meta("deploymentId")} is created")
  ...
  def getInstanceStatus(deploymentId: String): Map[String, String] = {
    ...
    items.map(item => {
      val status = jsonString.extract[String]('resources/ item/ element(0)/ 'info/ 'status)
      item -> status
    }).toMap
  }
  ...
  def getKibanaUrl(deploymentId: String): String = {
    val jsonString = getDeploymentStateInfo(deploymentId)
    jsonString.extract[String]('resources/ 'kibana/ element(0)/ 'info/ 'metadata/ 'service_url)
  }
  ...
  def waitForClusterToStart(deploymentId: String) = {
    ...
    logger.info(s"waitForClusterToStart: waitTime ${waitTime}ms, poolingInterval ${poolingInterval}ms")
    while(!started && poolingInterval > 0) {
      ...
      if (statuses.values.filter(s => s != "started").size == 0) {
        ...
        logger.info(s"waitForClusterToStart: Deployment is in progress... ${statuses.toString()}")
    ...
    if (!started) {
      throw new RuntimeException(s"Deployment ${deploymentId} was not ready after ${waitTime} ms")
    }
  }
  ...
  def deleteDeployment(id: String): Unit = {
    ...
    logger.info(s"deleteDeployment: Finished with status code ${response.getStatusLine.getStatusCode}")
  }
